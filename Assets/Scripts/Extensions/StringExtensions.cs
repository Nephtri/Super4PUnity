...
{
    public static string CleanFolderPath(this string str)
    {
        while(str.Contains(".."))
        {
            var backIndex = str.IndexOf("/..");
            var prevIndex = str.LastIndexOf('/', backIndex-1);
            str = str.Remove(prevIndex, backIndex - prevIndex + 3);
        }
        return str;
    }

    public static void CrossFadeAlpha(this CanvasGroup canvasGroup, MonoBehaviour monoCtrl, float finalAlpha, float duration, Action action)
    {
        monoCtrl.StartCoroutine(canvasGroup.CrossFadeCR(finalAlpha, duration, finalAlpha > canvasGroup.alpha, action));
    }

    private static IEnumerator CrossFadeCR(this CanvasGroup canvasGroup, float finalAlpha, float duration, bool isFadeIn, Action action)
    {
